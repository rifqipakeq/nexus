...
class _QuizScreenState extends ConsumerState<QuizScreen>
  ...
  Future<void> _finishQuiz() async {
    final quizService = ref.read(quizServiceProvider);
    final earned =
        quizService.calculateTokens(correct: _correctCount, streak: _maxStreak);

    setState(() {
      _quizComplete = true;
      _tokensEarned = earned;
      _isAwarding = true;
    });

    final newTotal = await quizService.awardTokens(earned);

    ref.read(quizTokensProvider.notifier).state = newTotal;
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          Text(grade,
              style: TextStyle(
                  fontSize: 32, color: gradeColor, fontWeight: FontWeight.bold)),
                ...
                : Column(
                    children: [
                      ...
                      LinearProgressIndicator(
                        value: (quizTokens / 100).clamp(0.0, 1.0),
                        backgroundColor: Colors.white12,
  ...
  Widget build(BuildContext context) {
    ...
    if (answered) {
      if (index == correctIndex) {
        ...
        trailingIcon = const Icon(Icons.check_circle, color: Color(0xFF4CAF50), size: 20);
        ...
        children: [
          Row(
            children: [
              ...
              const SizedBox(width: 6),
              Text(
                isCorrect ? 'Bener banget! Ini Penjelasannya:' : 'Belajar dulu yahh:',
                style: TextStyle(
                  color: color,
                  fontSize: 12,
                  fontWeight: FontWeight.bold,
